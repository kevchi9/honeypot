...
async def start_log_collector(run_event: threading.Event, LOG_PATH, logger):
    ...
    logger.debug('log_collector starting...')
    ...
    try:
        ...
        last_logs = read_logs(LOG_PATH, logger)
    except FileNotFoundError as e:
        raise

    # loop until the thread is killed
    logger.info("log_collector running.") 
    while(run_event.is_set()):
        ...
        if check_if_modified(last_modify, LOG_PATH, logger):
            ...
            # await mqtt_publisher.connect_and_send_to_broker( "10.0.21.183", 1883, logger, new_log)
